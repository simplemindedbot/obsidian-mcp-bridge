...
export class SettingsMigration {
   ...
   */
  async migrateSettings(settings: MCPBridgeSettings): Promise<MCPBridgeSettings> {
    ...
    console.log(`MCP Bridge: Migrating settings from ${currentVersion} to ${CURRENT_SETTINGS_VERSION}`);
   ...
   */
  private async migrateToV0_2_0(settings: MCPBridgeSettings): Promise<MCPBridgeSettings> {
    console.log('MCP Bridge: Running migration to v0.2.0 (workingDirectory support)');
    ...
    if (settings.servers?.filesystem) {
      ...
      if (config.args && config.args.length > 0) {
        ...
        if (lastArg === './' || lastArg.startsWith('/') || lastArg.startsWith('~') || lastArg.includes('/')) {
          ...
          console.log(`MCP Bridge: Moved directory '${lastArg}' from args to workingDirectory`);
        }
      }
      ...
      if (!config.workingDirectory) {
        try {
          ...
          console.log(`MCP Bridge: Set default workingDirectory to vault path: ${vaultPath}`);
        } catch (error) {
          console.warn('MCP Bridge: Could not determine vault path, using fallback');
    ...
    for (const [serverId, serverConfig] of Object.entries(settings.servers || {})) {
      if (serverId !== 'filesystem' && serverConfig.args) {
        const lastArg = serverConfig.args[serverConfig.args.length - 1];
        if (lastArg && (lastArg.startsWith('/') || lastArg.startsWith('~') || lastArg === './')) {
          serverConfig.workingDirectory = lastArg;
          serverConfig.args = serverConfig.args.slice(0, -1);
          console.log(`MCP Bridge: Migrated ${serverId} server workingDirectory`);
        }
      }
    }
